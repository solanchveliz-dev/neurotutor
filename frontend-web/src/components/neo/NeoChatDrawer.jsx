...
function NeoChatDrawer() {
  ...
  return (
    <>
      {!isOpen && <NeoFloatingButton onClick={() => setIsOpen(true)} />}
      ...
      <section
        ...
        aria-hidden={!isOpen}
        className={`fixed inset-x-0 bottom-0 z-[70] flex h-[88dvh] max-h-[88dvh] flex-col overflow-hidden rounded-t-[28px] border border-white/80 bg-white shadow-[0_-24px_70px_rgba(30,58,138,0.25)] transition-transform duration-300 sm:inset-y-0 sm:left-auto sm:right-0 sm:h-auto sm:max-h-none sm:w-[430px] sm:rounded-none sm:rounded-l-[28px] ${isOpen ? "translate-y-0 sm:translate-x-0" : "translate-y-full sm:translate-x-full sm:translate-y-0"}`}
      >
        <header className="shrink-0 bg-gradient-to-br from-[#DFF4FF] via-white to-[#EDE9FE] px-5 pb-5 pt-4">
        ...
        <div className="shrink-0 border-y border-[#E5E7EB] bg-white px-4 py-3">
        ...
        <form className="sticky bottom-0 z-20 shrink-0 border-t border-[#E5E7EB] bg-white/98 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[0_-10px_24px_rgba(30,58,138,0.08)] backdrop-blur" onSubmit={(event) => { event.preventDefault(); sendMessage(input); }}>
          <div className="flex items-end gap-2 rounded-[22px] border border-[#BFDBFE] bg-[#F8FBFF] p-2 focus-within:border-[#7C3AED] focus-within:ring-4 focus-within:ring-[#A78BFA]/15">
            ...
            <textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={handleInputKeyDown} rows={1} maxLength={900} placeholder="Pregúntale a NEO..." className="max-h-28 min-h-10 flex-1 resize-none bg-transparent px-1 py-2 text-sm font-semibold text-[#1E3A8A] outline-none placeholder:text-[#94A3B8]" />
            <Button type="submit" size="icon" aria-label="Enviar mensaje" disabled={!input.trim() || isLoading} className="size-10 shrink-0 rounded-full bg-gradient-to-br from-[#2563EB] to-[#7C3AED] text-white shadow-md"><Send className="size-4" /></Button>
          </div>
        </form>
      </section>
    </>
