...
function EquivalentFraction({ value }) {
  return <span className="inline-flex items-center gap-1">{value?.whole && <strong className="text-xl font-black">{value.whole}</strong>}<StackedFraction numerator={value?.numerator} denominator={value?.denominator} className="text-current" /></span>;
}
...
function MultiplicationVisual({ visual }) {
  if (visual.kind === "circle") return <FractionCircle numerator={visual.numerator} denominator={visual.denominator} color={visual.color} />;
  const total = Number(visual.denominator) || 1;
  const columns = visual.kind === "grid" ? (total % 4 === 0 ? 4 : 3) : total;
  return <div className="grid h-16 w-24 overflow-hidden rounded-sm border-2 border-slate-500 bg-white" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>{Array.from({ length: total }, (_, index) => <span key={index} className="border-b border-r border-slate-400" style={{ backgroundColor: index < Number(visual.numerator) ? visual.color : "white" }} />)}</div>;
}

function MultiplicationExampleCard({ item }) {
  const tone = item.tone === "violet" ? "border-violet-200 bg-violet-50/60 text-violet-700" : "border-blue-200 bg-sky-50/60 text-blue-600";
  return <article className={`rounded-[22px] border p-4 shadow-sm ${tone}`}><div className="flex items-center justify-between gap-3"><h3 className="flex items-center gap-2 font-black"><span className="grid size-8 place-items-center rounded-full bg-current text-white"><span className="text-white">{item.number}</span></span>{item.title}</h3><div className="flex items-center gap-2"><EquivalentFraction value={item.left} /><span className="font-black">×</span><EquivalentFraction value={item.right} /><span className="font-black">=</span><EquivalentFraction value={item.operation} />{item.intermediate && <><span className="font-black">=</span><EquivalentFraction value={item.intermediate} /></>}<span className="font-black">=</span><EquivalentFraction value={item.result} /></div></div><div className="mt-4 grid items-start gap-2 sm:grid-cols-[1fr_auto_1fr_auto_1fr]">{item.visuals?.map((visual, index) => <div key={`${visual.kind}-${index}`} className="contents"><div className="grid place-items-center text-center"><MultiplicationVisual visual={visual} /><p className="mt-2 text-[11px] font-semibold leading-4 text-slate-700">{visual.caption}</p></div>{index < item.visuals.length - 1 && <span className="self-center text-lg font-black text-nt-text-primary">{index === item.visuals.length - 2 ? "=" : "×"}</span>}</div>)}</div><div className="mt-3 flex items-center justify-end gap-2 rounded-[14px] bg-emerald-50 px-3 py-2 text-emerald-700"><strong className="text-xs">Resultado:</strong><EquivalentFraction value={item.result} /></div></article>;
}

function FractionMultiplicationContent({ sections }) {
  const section = (type) => sections.find((item) => item.type === type);
  const idea = section("multiplication_key_idea");
  const examples = section("multiplication_examples");
  const realLife = section("real_life_multiplication");
  const smaller = section("smaller_result");
  const mistakes = section("multiplication_mistakes");
  const summary = section("multiplication_summary");

  return <div className="mt-3 grid gap-3">
    {idea && <section className="grid items-center gap-3 rounded-[20px] border border-amber-200 bg-amber-50/80 px-4 py-3 shadow-sm lg:grid-cols-[32px_minmax(0,1fr)_auto_60px]"><Star className="size-7 fill-amber-400 text-amber-400" /><div><h2 className="font-black text-amber-900">{idea.title}</h2><p className="mt-1 text-sm font-semibold text-slate-700">{idea.text}</p></div><div className="flex items-center gap-2 rounded-2xl bg-white/60 px-4 py-2 text-emerald-700"><EquivalentFraction value={idea.formula?.left} /><span className="font-black text-red-500">×</span><EquivalentFraction value={idea.formula?.right} /><span className="font-black text-nt-text-primary">=</span><EquivalentFraction value={idea.formula?.result} /></div><img src={assetUrl(idea.image)} alt="Idea clave" className="h-12 w-full object-contain" /></section>}
    {examples && <section><h2 className="flex items-center gap-2 text-lg font-black text-nt-blue"><BookOpen className="size-5" />{examples.title}</h2><div className="mt-2 grid gap-3 lg:grid-cols-2">{examples.items?.map((item) => <MultiplicationExampleCard key={item.number} item={item} />)}</div></section>}
    {realLife && <section><h2 className="flex items-center gap-2 font-black text-nt-blue"><Target className="size-5" />{realLife.title}</h2><div className="mt-2 grid items-center gap-4 rounded-[22px] border border-sky-200 bg-sky-50/60 p-4 shadow-sm lg:grid-cols-[1.1fr_1.2fr_.75fr_110px]"><div><div className="flex flex-wrap items-center gap-1.5 text-xs font-semibold leading-5 text-slate-700"><span>{realLife.prefix}</span><EquivalentFraction value={realLife.first} /><span>{realLife.middle}</span><EquivalentFraction value={realLife.second} /><span>{realLife.suffix}</span></div><p className="mt-2 text-xs font-black text-nt-text-primary">{realLife.question}</p></div><div className="flex items-center justify-center gap-2">{realLife.images?.map((image, index) => <div key={`${image}-${index}`} className="contents"><img src={assetUrl(image)} alt="Paso del ejemplo con una torta" className="h-20 min-w-0 flex-1 object-contain" />{index < realLife.images.length - 1 && <ArrowRight className="size-4 shrink-0 text-slate-400" />}</div>)}</div><div><FractionEquation values={realLife.equation} /><p className="mt-2 rounded-xl border border-blue-200 bg-blue-100/70 p-2 text-xs font-semibold leading-5 text-nt-text-primary">{realLife.message}</p></div><img src={assetUrl(realLife.neo)} alt="NEO explicando el ejemplo" className="mx-auto h-28 w-full object-contain drop-shadow-md" /></div></section>}
    <div className="grid gap-3 lg:grid-cols-[1.05fr_1.15fr_.8fr]">
      {smaller && <section className="rounded-[22px] border border-emerald-200 bg-emerald-50/70 p-4 shadow-sm"><h2 className="flex items-center gap-2 font-black text-emerald-700"><CheckCircle2 className="size-5" />{smaller.title}</h2>{smaller.paragraphs?.map((paragraph) => <p key={paragraph} className="mt-2 text-xs font-semibold leading-5 text-slate-700">{paragraph}</p>)}<div className="mt-3"><FractionEquation values={smaller.equation} /></div></section>}
      {mistakes && <section className="relative overflow-hidden rounded-[22px] border border-red-200 bg-red-50/70 p-4 shadow-sm"><div className="max-w-[75%]"><h2 className="flex items-center gap-2 font-black text-red-600"><AlertTriangle className="size-5" />{mistakes.title}</h2><div className="mt-3 grid gap-2">{mistakes.items?.map((item) => <p key={item} className="flex gap-2 text-xs font-semibold leading-5 text-slate-700"><span className="font-black text-red-500">×</span>{item}</p>)}</div></div><img src={assetUrl(mistakes.image)} alt="NEO señalando errores" className="absolute bottom-0 right-1 h-28 w-[27%] object-contain" /></section>}
      {summary && <section className="rounded-[22px] border border-blue-200 bg-blue-50/70 p-4 shadow-sm"><h2 className="font-black text-blue-700">{summary.title}</h2><div className="mt-3 grid gap-2">{summary.items?.map((item) => <p key={item} className="flex gap-2 text-xs font-semibold leading-5 text-slate-700"><Check className="size-4 shrink-0 text-green-500" strokeWidth={3} />{item}</p>)}</div><img src={assetUrl(summary.image)} alt="Libros" className="ml-auto mt-2 h-14 w-24 object-contain" /></section>}
    </div>
  </div>;
}

function FractionDivisionContent({ sections }) {
  const section = (type) => sections.find((item) => item.type === type);
  const idea = section("division_key_idea");
  const steps = section("division_steps");
  const examples = section("division_examples");
  const realLife = section("division_real_life");
  const tips = section("division_tips");
  const mistakes = section("division_mistakes");
  const tones = { blue: "border-blue-200 bg-sky-50/60 text-blue-600", green: "border-emerald-200 bg-emerald-50/60 text-emerald-700", violet: "border-violet-200 bg-violet-50/60 text-violet-700", orange: "border-orange-200 bg-orange-50/60 text-orange-600", sky: "border-sky-200 bg-blue-50/60 text-sky-600" };

  return <div className="mt-3 grid gap-3">
    {idea && <section className="grid items-center gap-3 rounded-[20px] border border-amber-200 bg-amber-50/80 px-4 py-3 shadow-sm lg:grid-cols-[32px_minmax(0,1fr)_auto_60px]"><Star className="size-7 fill-amber-400 text-amber-400" /><div><h2 className="font-black text-amber-900">{idea.title}</h2><p className="mt-1 text-sm font-semibold leading-5 text-slate-700">{idea.text}</p></div><div className="rounded-2xl bg-white/60 px-4 py-2 text-emerald-700"><FractionEquation values={idea.formula} /></div><img src={assetUrl(idea.image)} alt="Idea clave" className="h-12 w-full object-contain" /></section>}
    {steps && <section><h2 className="flex items-center gap-2 text-lg font-black text-nt-blue"><BookOpen className="size-5" />{steps.title}</h2><div className="mt-2 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{steps.items?.map((item) => <article key={item.number} className={`grid min-h-56 content-between rounded-[22px] border p-3 shadow-sm ${tones[item.tone]}`}><div><h3 className="flex items-start gap-2 text-sm font-black leading-5"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-current text-white"><span className="text-white">{item.number}</span></span>{item.title}</h3>{item.text && <p className="mt-3 text-xs font-semibold leading-5 text-slate-700">{item.text}</p>}</div>{item.formula && <FractionEquation values={item.formula} />}<div>{item.example && <><p className="mb-1 text-xs font-black text-slate-700">{steps.exampleLabel}</p><FractionEquation values={item.example} /></>}{item.result && <p className="rounded-xl bg-emerald-50 px-2 py-2 text-center text-xs font-bold leading-5 text-emerald-800">{item.result}</p>}</div></article>)}</div></section>}
    {(examples || realLife) && <section><h2 className="flex items-center gap-2 text-lg font-black text-nt-blue"><BookOpen className="size-5" />{examples?.title}</h2><div className="mt-2 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{examples?.items?.map((item) => <article key={item.title} className={`grid min-h-48 content-between rounded-[22px] border p-3 text-center shadow-sm ${tones[item.tone]}`}><h3 className="font-black">{item.title}</h3><FractionEquation values={item.initial} /><FractionEquation values={item.solution} /><div className="flex items-center justify-center gap-2 rounded-xl bg-emerald-50 px-2 py-2 text-emerald-700"><strong className="text-xs">{examples.resultLabel}</strong><FractionEquation values={[item.result]} /></div></article>)}{realLife && <article className="relative min-h-48 overflow-hidden rounded-[22px] border border-blue-200 bg-blue-50/70 p-3 shadow-sm"><div className="max-w-[72%]"><h3 className="font-black text-blue-700">{realLife.title}</h3><div className="mt-2 flex flex-wrap items-center gap-1 text-xs font-semibold leading-5 text-slate-700"><span>{realLife.prefix}</span><EquivalentFraction value={realLife.amount} /><span>{realLife.middle}</span><EquivalentFraction value={realLife.portion} /><span>{realLife.suffix}</span></div><FractionEquation values={realLife.equation} /><p className="text-xs font-black text-nt-text-primary">{realLife.result}</p></div><img src={assetUrl(realLife.image)} alt="Vaso de jugo" className="absolute bottom-2 right-2 h-28 w-[28%] object-contain" /></article>}</div></section>}
    <div className="grid gap-3 lg:grid-cols-2">
      {tips && <section className="relative overflow-hidden rounded-[22px] border border-emerald-200 bg-emerald-50/70 p-4 shadow-sm"><div className="max-w-[76%]"><h2 className="flex items-center gap-2 font-black text-emerald-700"><CheckCircle2 className="size-5" />{tips.title}</h2><div className="mt-3 grid gap-2">{tips.items?.map((item) => <p key={item} className="flex gap-2 text-xs font-semibold leading-5 text-slate-700"><Check className="size-4 shrink-0 text-green-500" strokeWidth={3} />{item}</p>)}</div></div><img src={assetUrl(tips.image)} alt="NEO compartiendo consejos" className="absolute bottom-0 right-1 h-28 w-[24%] object-contain" /></section>}
      {mistakes && <section className="relative overflow-hidden rounded-[22px] border border-red-200 bg-red-50/70 p-4 shadow-sm"><div className="max-w-[75%]"><h2 className="flex items-center gap-2 font-black text-red-600"><AlertTriangle className="size-5" />{mistakes.title}</h2><div className="mt-3 grid gap-2">{mistakes.items?.map((item) => <p key={item} className="flex gap-2 text-xs font-semibold leading-5 text-slate-700"><span className="font-black text-red-500">×</span>{item}</p>)}</div></div><img src={assetUrl(mistakes.image)} alt="NEO señalando errores" className="absolute bottom-0 right-1 h-28 w-[25%] object-contain" /></section>}
    </div>
  </div>;
}

function AdvancedWelcomeContent({ sections }) {
  const section = (type) => sections.find((item) => item.type === type);
  const review = section("advanced_review");
  const uses = section("advanced_real_life");
  const think = section("advanced_think");
  const goals = section("advanced_goals");
  const tones = { blue: "border-blue-200 bg-sky-50/70", green: "border-emerald-200 bg-emerald-50/70", violet: "border-violet-200 bg-violet-50/70", amber: "border-amber-200 bg-amber-50/70", rose: "border-rose-200 bg-rose-50/70" };
  const operationTones = ["bg-blue-500", "bg-green-500", "bg-violet-500", "bg-orange-500"];

  return <div className="mt-3 grid gap-3">
    {review && <section className="grid items-center gap-3 rounded-[22px] border border-amber-200 bg-amber-50/70 p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_auto_90px]"><div><h2 className="flex items-center gap-2 font-black text-nt-text-primary"><Lightbulb className="size-6 text-amber-400" />{review.title}</h2>{review.paragraphs?.map((paragraph) => <p key={paragraph} className="mt-1 text-sm font-semibold leading-5 text-slate-700">{paragraph}</p>)}</div><div className="flex flex-wrap justify-center gap-4">{review.operations?.map((operation, index) => <span key={operation} className={`grid size-12 place-items-center rounded-full text-3xl font-black text-white shadow-md ${operationTones[index % operationTones.length]}`}>{operation}</span>)}</div><img src={assetUrl(review.image)} alt="Cerebro aprendiendo" className="mx-auto h-20 w-full object-contain" /></section>}
    {uses && <section><h2 className="flex items-center gap-2 text-lg font-black text-nt-blue"><BookOpen className="size-5" />{uses.title}</h2><div className="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{uses.items?.map((item) => <article key={item.title} className={`grid min-h-48 content-between rounded-[22px] border p-3 text-center shadow-sm ${tones[item.tone]}`}><img src={assetUrl(item.image)} alt={item.title} className="mx-auto h-24 w-full object-contain drop-shadow-sm" /><div><h3 className="font-black text-nt-text-primary">{item.title}</h3><p className="mt-2 text-xs font-semibold leading-5 text-slate-700">{item.text}</p></div></article>)}</div></section>}
    <div className="grid gap-3 lg:grid-cols-[1.2fr_1fr]">
      {think && <section className="relative overflow-hidden rounded-[22px] border border-emerald-200 bg-emerald-50/60 p-4 shadow-sm"><div className="max-w-[72%]"><h2 className="flex items-center gap-2 font-black text-emerald-700"><Target className="size-5 text-red-500" />{think.title}</h2><p className="mt-2 text-xs font-semibold leading-5 text-slate-700">{think.intro}</p>{think.questions?.map((question) => <p key={question} className="mt-1 text-xs font-black text-nt-text-primary">{question}</p>)}<p className="mt-2 text-xs font-semibold text-slate-700">{think.conclusion}</p></div><img src={assetUrl(think.image)} alt="NEO pensando" className="absolute bottom-0 right-2 h-36 w-[30%] object-contain drop-shadow-md" /></section>}
      {goals && <section className="grid items-center gap-3 rounded-[22px] border border-violet-200 bg-violet-50/60 p-4 shadow-sm sm:grid-cols-[minmax(0,1fr)_120px]"><div><h2 className="flex items-center gap-2 font-black text-violet-700"><Star className="size-5 fill-amber-400 text-amber-400" />{goals.title}</h2><div className="mt-3 grid gap-2">{goals.items?.map((item) => <p key={item} className="flex gap-2 text-xs font-semibold text-slate-700"><CheckCircle2 className="size-4 shrink-0 text-violet-600" />{item}</p>)}</div></div><img src={assetUrl(goals.image)} alt="Trofeo" className="mx-auto h-28 w-full object-contain drop-shadow-md" /></section>}
    </div>
  </div>;
}

function SimplificationBar({ visual, className = "w-full" }) {
  const total = Math.max(1, Number(visual?.denominator) || 1);
  const filled = Math.min(total, Math.max(0, Number(visual?.numerator) || 0));
  return <div className={`grid h-10 overflow-hidden rounded-sm border-2 border-slate-500 bg-white ${className}`} style={{ gridTemplateColumns: `repeat(${total}, minmax(0, 1fr))` }}>{Array.from({ length: total }, (_, index) => <span key={index} className="border-r border-slate-400 last:border-r-0" style={{ backgroundColor: index < filled ? visual.color : "white" }} />)}</div>;
}

function AdvancedSimplificationContent({ sections }) {
  const section = (type) => sections.find((item) => item.type === type);
  const idea = section("simplification_key_idea");
  const steps = section("simplification_steps");
  const examples = section("simplification_examples");
  const irreducible = section("irreducible_fraction");
  const mistakes = section("simplification_mistakes");
  const summary = section("simplification_summary");
  const tones = { green: "border-emerald-200 bg-emerald-50/60 text-emerald-700", blue: "border-blue-200 bg-sky-50/60 text-blue-600", violet: "border-violet-200 bg-violet-50/60 text-violet-700", orange: "border-orange-200 bg-orange-50/60 text-orange-600" };

  return <div className="mt-3 grid gap-3">
    {idea && <section className="grid items-center gap-3 rounded-[20px] border border-amber-200 bg-amber-50/80 px-4 py-3 shadow-sm sm:grid-cols-[32px_minmax(0,1fr)_70px]"><Star className="size-7 fill-amber-400 text-amber-400" /><div><h2 className="font-black text-amber-900">{idea.title}</h2><p className="mt-1 text-sm font-semibold leading-5 text-slate-700">{idea.text}</p></div><img src={assetUrl(idea.image)} alt="Idea clave" className="h-14 w-full object-contain" /></section>}
    {steps && <section><h2 className="flex items-center gap-2 text-lg font-black text-nt-blue"><BookOpen className="size-5" />{steps.title}<EquivalentFraction value={steps.titleFraction} /></h2><div className="mt-2 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{steps.items?.map((item) => <article key={item.number} className={`grid min-h-56 content-between rounded-[22px] border p-3 shadow-sm ${tones[item.tone]}`}><h3 className="flex items-start gap-2 text-sm font-black leading-5"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-current text-white"><span className="text-white">{item.number}</span></span>{item.title}</h3>{item.fraction && <div className="grid gap-3"><div className="mx-auto"><EquivalentFraction value={item.fraction} /></div><SimplificationBar visual={item.visual} /></div>}{item.text && <p className="text-center text-xs font-semibold leading-5 text-slate-700">{item.text}</p>}{item.calculation && <p className="text-center text-base font-black text-nt-text-primary">{item.calculation}</p>}{item.operation && <div className="flex items-center justify-center gap-4"><EquivalentFraction value={item.operation} /><ArrowRight className="size-5" /><EquivalentFraction value={item.result} /></div>}{item.resultLabel && <p className="text-center text-xs font-semibold text-slate-700">{item.resultLabel}</p>}{item.equation && <FractionEquation values={item.equation} />}</article>)}</div></section>}
    {(examples || irreducible) && <section><h2 className="flex items-center gap-2 text-lg font-black text-nt-blue"><BookOpen className="size-5" />{examples?.title}</h2><div className="mt-2 grid gap-3 sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_1.1fr]">{examples?.items?.map((item, index) => <article key={index} className="grid min-h-44 content-between rounded-[22px] border border-violet-200 bg-violet-50/60 p-3 shadow-sm"><div className="flex items-center justify-center gap-2 text-violet-700"><EquivalentFraction value={{ numerator: `${item.from.numerator} ÷ ${item.divisor}`, denominator: `${item.from.denominator} ÷ ${item.divisor}` }} /><ArrowRight className="size-4" /><EquivalentFraction value={item.to} /></div><div className="flex items-center justify-center gap-2"><SimplificationBar visual={item.visualFrom} className="w-24" /><span className="font-black text-violet-600">÷ {item.divisor}</span><SimplificationBar visual={item.visualTo} className="w-16" /></div></article>)}{irreducible && <aside className="relative min-h-44 overflow-hidden rounded-[22px] border border-blue-200 bg-blue-50/70 p-4 shadow-sm"><div className="max-w-[70%]"><h3 className="font-black leading-5 text-blue-700">{irreducible.title}</h3><p className="mt-2 text-xs font-semibold leading-5 text-slate-700">{irreducible.text}</p><div className="mt-2 flex items-center gap-2 text-xs font-semibold text-slate-700"><strong>{irreducible.exampleLabel}</strong><EquivalentFraction value={irreducible.example} /><span>{irreducible.exampleText}</span></div></div><img src={assetUrl(irreducible.image)} alt="NEO explicando" className="absolute bottom-0 right-1 h-28 w-[30%] object-contain" /></aside>}</div></section>}
    <div className="grid gap-3 lg:grid-cols-2">
      {mistakes && <section className="rounded-[22px] border border-red-200 bg-red-50/70 p-4 shadow-sm"><h2 className="flex items-center gap-2 font-black text-red-600"><AlertTriangle className="size-5" />{mistakes.title}</h2><div className="mt-3 grid gap-2">{mistakes.items?.map((item) => <div key={item.text} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2"><p className="flex gap-2 text-xs font-semibold text-slate-700"><span className="font-black text-red-500">×</span>{item.text}</p><FractionEquation values={item.equation} /><span className="text-xs font-black text-red-500">×</span></div>)}</div></section>}
      {summary && <section className="grid items-center gap-3 rounded-[22px] border border-violet-200 bg-violet-50/60 p-4 shadow-sm sm:grid-cols-[minmax(0,1fr)_130px]"><div><h2 className="font-black text-violet-700">{summary.title}</h2><div className="mt-3 grid gap-2">{summary.items?.map((item) => <p key={item} className="flex gap-2 text-xs font-semibold text-slate-700"><Check className="size-4 shrink-0 text-green-500" strokeWidth={3} />{item}</p>)}</div></div><img src={assetUrl(summary.image)} alt="Libros de estudio" className="mx-auto h-24 w-full object-contain" /></section>}
    </div>
  </div>;
}
...
function TheoryLesson() {
  ...
  const isIntermediateFractionMultiplication = webContent?.layout === "intermediate_fraction_multiplication";
  ...
  return (
    <StudentLayout
      ...
      rightPanel={
        <LessonSidebar lesson={lesson} lessons={lessons} position={position} progress={moduleProgress} neoTip={neoTip} nextLessonData={webContent?.nextLesson} showNeoCard={!webContent || Boolean(neoTip)} />
      }
    >
      <article className="rounded-nt-card border border-white/85 bg-white/95 p-3 shadow-nt-card sm:p-4 lg:p-5">
        ...
        {isIntermediateFractionMultiplication && <FractionMultiplicationContent sections={webSections} />}

        {isIntermediateFractionDivision && <FractionDivisionContent sections={webSections} />}

        {isAdvancedWelcome && <AdvancedWelcomeContent sections={webSections} />}
      ...
      <section className="flex flex-col gap-3 rounded-[26px] border border-white/85 bg-white/90 p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        {nextLesson ? (
          ...
          <Button
            ...
            className={`h-12 rounded-[18px] px-6 font-black text-white ${webContent?.finalAction ? "bg-gradient-to-r from-nt-blue to-nt-purple" : "bg-gradient-to-r from-nt-green to-emerald-500"}`}
            onClick={completeTheory}
          >
            {isCompleting ? (webContent?.finalAction?.loadingLabel ?? "Guardando progreso...") : (webContent?.finalAction?.label ?? "Finalizar teoría e ir a práctica")}
